...
class MainWindow(QMainWindow):
    ...
    def update_status(self, message: str, is_error: bool = False) -> None:
        ...
        if is_error:
            ...
            QTimer.singleShot(
                3000, lambda: self.status_label.setStyleSheet(STYLES["status_label"])
            )
            logger.info(message)
    ...
    def remove_shortcut(self) -> None:
        ...
        try:
            ...
            for item in selected_items:
                ...
                if os.path.exists(shortcut_path):
                    try:
                        ...
                        self.shortcuts_listbox.takeItem(
                            self.shortcuts_listbox.row(item)
                        )
                        removed_count += 1
            ...
            if removed_count > 0:
                ...
                QTimer.singleShot(
                    3000,
                    lambda: self.status_label.setStyleSheet(STYLES["status_label"]),
                )
    ...
    def load_wsl_applications(self) -> None:
        ...
        try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    for file_path in desktop_files:
                        ...
                        name_result = subprocess.run(
                            name_cmd, capture_output=True, text=True
                        )

                        logger.debug(
                            f"Name result for {file_path}: {name_result.stdout}"
                        )
            ...
            if apps_found > 0:
                self.update_status(
                    f"Found {apps_found} WSL application{'s' if apps_found != 1 else ''}"
                )
            else:
                self.update_status(
                    "No WSL applications found. Try installing some GUI applications in WSL.",
                    True,
                )
        ...
        try:
            ...
            if dialog.exec_():  # Dialog accepted if result is non-zero
                ...
                if not name or not command:
                    self.update_status(
                        "Application name and command are required", True
                    )
                    return
    ...
    def create_shortcut(self):
        ...
        try:
            shortcut_dir = os.path.expandvars(
                f"%AppData%\\Microsoft\\Windows\\Start Menu\\Programs\\{self.distro_name}"
            )
            ...
            for item in selected_items:
                ...
                with open("create_shortcut.vbs", "w") as f:
                    ...
                    icon_location = (
                        icon_path if icon_path else "C:\\Program Files\\WSL\\wslg.exe,0"
                    )
                ...
                subprocess.run(
                    ["cscript", "//Nologo", "create_shortcut.vbs"], check=True
                )
    ...
    def get_wsl_distro_info(self):
        ...
        try:
            ...
            cmd = ["wsl.exe", "-l", "-v"]
            result = subprocess.run(
                cmd, capture_output=True, text=True, encoding="utf-16le"
            )
    ...
    def _create_list_section(
        self, title: str, select_multiple: bool = False
    ) -> Dict[str, Union[QVBoxLayout, QListWidget]]:
        """
